...
RSpec.describe GitHooks::Checks::PreCommit::RuboCop do
  let(:config) { described_class.definition.default_config.merge('command' => %w[bundle exec rubocop]) }
  let(:repo_root) { Dir.mktmpdir('rubocop_spec') }
  ...
  describe '#run' do
    ...
    context 'when command succeeds' do
      let(:ruby_path) { File.join(repo_root, 'lib/foo.rb') }
      let(:files) { [ruby_path] }

      before do
        FileUtils.mkdir_p(File.dirname(ruby_path))
        File.write(ruby_path, 'puts 1')
    ...
    context 'when command fails' do
      ...
      before do
        FileUtils.mkdir_p(File.dirname(ruby_path))
        File.write(ruby_path, 'puts 1')
        allow(Open3).to receive(:capture2e).and_return(["Offense in lib/foo.rb\n", double(success?: false)])
      end
      ...
      it 'returns fail with command output' do
        ...
        expect(result.messages).to include('Offense in lib/foo.rb')
      end
    end
  end
end
