...
RSpec.describe GitHooks::Checks::PreCommit::Golint do
  let(:config) { described_class.definition.default_config.merge('command' => %w[golint]) }
  let(:repo_root) { Dir.mktmpdir('golint_spec') }
    ...
    context 'when command fails' do
      ...
      before do
        ...
        allow(Open3).to receive(:capture2e)
                    .and_return(["main.go:2:1: don't use an underscore in package name\n", double(success?: false)])
      end
      ...
      it 'returns fail with command output' do
        ...
        expect(result.messages).to include("main.go:2:1: don't use an underscore in package name")
      end
    end
  end
end
