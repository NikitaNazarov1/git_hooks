...
RSpec.describe GitHooks::Checks::PreCommit::Eslint do
  let(:config) { described_class.definition.default_config.merge('command' => %w[eslint]) }
  let(:repo_root) { Dir.mktmpdir('eslint_spec') }
  ...
  describe '#run' do
    ...
    context 'when command fails' do
      ...
      before do
        FileUtils.mkdir_p(File.dirname(js_path))
        File.write(js_path, 'console.log("ok");')
        allow(Open3).to receive(:capture2e).and_return(["1:7 error Unexpected console statement\n", double(success?: false)])
      end
      ...
      it 'returns fail with command output' do
        ...
        expect(result.messages).to include('1:7 error Unexpected console statement')
      end
    end
  end
end
