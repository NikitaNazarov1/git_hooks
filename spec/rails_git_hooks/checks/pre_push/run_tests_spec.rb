...
RSpec.describe GitHooks::Checks::PrePush::RunTests do
  let(:config) { described_class.definition.default_config.merge('command' => %w[bundle exec rspec]) }
  ...
  describe '#run' do
    ...
    context 'when command fails' do
      before do
        allow(Open3).to receive(:capture2e).and_return(["Failure:\n  expected 1 to eq 2\n", double(success?: false)])
      end
      ...
      it 'returns fail with command output' do
        ...
        expect(result.messages).to include(match(/expected|Failure/))
      end
    end
  end
end
