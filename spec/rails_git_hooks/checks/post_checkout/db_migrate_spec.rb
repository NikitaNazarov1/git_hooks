...
RSpec.describe GitHooks::Checks::PostCheckout::DbMigrate do
  ...
  describe '#run' do
    ...
    context 'when migration file in applicable set and command succeeds' do
      let(:files) { ['db/migrate/20240101000000_add_column.rb'] }
    ...
    context 'when schema changed and command fails' do
      let(:files) { ['db/schema.rb'] }

      before do
        allow(Open3).to receive(:capture2e).and_return(["Migration error\n", double(success?: false)])
      end
      ...
      it 'returns fail with output' do
        ...
        expect(result.messages).to include('Migration error')
      end
    end
  end
end
